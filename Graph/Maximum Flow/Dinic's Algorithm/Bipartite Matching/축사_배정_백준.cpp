...
int d_dfs(int node, int fin, int flow){
    ...
    for(int &i=chk[node];i<barn[node].size();i++){
        int next=barn[node][i];
...
int main()
{
    ...
    for(int i=1;i<=M;i++){
        barn[i+MAXN].push_back(MEM-1);
        barn[MEM-1].push_back(i+MAXN);
        c[i+MAXN][MEM-1]=1;
    }
    for(int i=1;i<=N;i++){
        scanf("%d", &a);
        for(int j=0;j<a;j++){
            scanf("%d", &b);
            barn[i].push_back(b+MAXN);
            barn[b+MAXN].push_back(i);
            c[i][b+MAXN]=1;
        }
    }
